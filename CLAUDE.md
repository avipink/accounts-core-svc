...
## Role
Domain owner for the Accounts bounded context. Spring Boot REST service (port 8081).

## Ecosystem
- Shared contracts: ../banking-contracts
- Orchestrator/specs: ../banking-platform/specs/
- BFF consumer: ../banking-bff (calls this service's API)
- Sibling core service: ../payments-core-svc (calls this service for account validation)

## Architecture Rules
- This service OWNS the Accounts domain — no other service writes account state
- Internal domain models (Account.kt) are PRIVATE — NEVER expose directly via API
- All API responses MUST map to contract types from banking-contracts at the boundary
- Internal fields (internalRiskScore, kycStatus, createdBy) must NEVER leak to API layer
- AccountMapper.kt is the ONLY place where internal → contract mapping occurs

## API Surface (port 8081)
...
- POST /api/v1/accounts/{id}/hold    → AccountResponse

## Mock Data
In-memory only — no database. AccountRepository seeds 5 realistic accounts on startup.
